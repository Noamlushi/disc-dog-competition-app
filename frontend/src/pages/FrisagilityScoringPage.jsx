...
export default function FrisagilityScoringPage() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            navigate(`/competition/${id}/start?run=Frisgility`);
        } catch (err) {
                ...
                <div className="container mx-auto px-4 py-4">
                    ...
                    <div className="grid grid-cols-2 gap-8 items-center">
                        <div className="text-center border-r border-gray-200">
                            <div className="text-sm text-gray-500 font-semibold mb-1">Total Score</div>
                            <div className="text-6xl font-black text-blue-900">{totalScore}</div>
                        </div>
                        <div className={`text-center ${timeLeft < 10000 && timeLeft > 0 ? 'text-red-600 animate-pulse' : 'text-gray-800'}`}>
                            <div className="text-sm text-gray-500 font-semibold mb-1">Time Left</div>
            ...
            <div className="container mx-auto px-4 py-6 max-w-2xl">
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                    <button
                        ...
                        className={`py-6 rounded-xl text-2xl font-bold text-white shadow-lg transition-transform ${isRunning || timeLeft === 0 ? 'bg-gray-300 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700 active:scale-95'
                            }`}
                    >
                    ...
                    <button
                        onClick={handleStop}
                        disabled={!isRunning}
                        className={`py-6 rounded-xl text-2xl font-bold text-white shadow-lg transition-transform ${!isRunning ? 'bg-gray-300 cursor-not-allowed' : 'bg-red-600 hover:bg-red-700 active:scale-95'
                            }`}
                    >
                        Stop Timer
                    </button>
                ...
                <div className="space-y-6 mb-8">
                    ...
                    {/* Obstacles Row */}
                    <div className="grid grid-cols-4 gap-3 bg-gray-100 p-4 rounded-xl">
                        ...
                        <div className="col-span-2 flex flex-col justify-end">
                            <button
                                ...
                                disabled={!isRunning || timeLeft === 0}
                                className={`h-32 rounded-xl text-2xl font-black text-white shadow-lg transition-all transform active:scale-[0.98] flex flex-col items-center justify-center border-b-4 border-emerald-800 ${!isRunning
                ...
                <div className="bg-white rounded-xl shadow p-4 mb-20">
                    <div className="flex items-center justify-between mb-4 border-b pb-2">
                        <h3 className="font-bold text-gray-700">Action History</h3>
                        <button
                            onClick={handleUndo}
                            disabled={actions.length === 0}
                            className="flex items-center gap-1 text-sm font-bold text-gray-500 hover:text-red-500 disabled:opacity-30"
                        >
                            <RotateCcw size={16} /> Undo Last Action
                        </button>
                    </div>
                    <div className="max-h-40 overflow-y-auto space-y-2">
                        {actions.length === 0 && <div className="text-center text-gray-300 py-4">No actions yet</div>}
                        {actions.map((action, idx) => (
                            <div key={action.id} className="flex justify-between items-center bg-gray-50 p-2 rounded text-sm">
                                <span className="text-gray-500 font-mono">#{actions.length - idx}</span>
                                <span className="font-medium text-gray-800">{action.name}</span>
                                <span className={`font-bold ${action.points > 0 ? 'text-green-600' : 'text-gray-400'}`}>
                                    +{action.points}
                                </span>
                            </div>
                        ))}
                ...
                <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t box-border">
                    <div className="container mx-auto max-w-2xl">
                        <button
                            onClick={handleSave}
                            className="w-full py-4 rounded-xl text-xl font-bold bg-gray-900 text-white shadow-lg hover:bg-gray-800 flex items-center justify-center gap-2"
                        >
